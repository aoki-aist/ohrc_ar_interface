...
// Calculate velocity from Acceleration sensors
public class XrBodyStatePublisher2 : MonoBehaviour
{
        ...
        InputFeatureUsage<Vector3> inputVelocity, InputFeatureUsage<Vector3> inputAngularVelocity, InputFeatureUsage<Vector3> inputPosition, InputFeatureUsage<Quaternion> inputRotation)
    {
        ...
        if (velocity != null)
        {
            poseTwistMsg.twist.linear.x = velocity.z;
            poseTwistMsg.twist.linear.y = -velocity.x;
            poseTwistMsg.twist.linear.z = velocity.y;
            Debug.Log(velocity.z);
        }
        if (angularVelocity != null)
        {
            poseTwistMsg.twist.angular.x = -angularVelocity.z;
            poseTwistMsg.twist.angular.y = angularVelocity.x;
            poseTwistMsg.twist.angular.z = -angularVelocity.y;
        }
